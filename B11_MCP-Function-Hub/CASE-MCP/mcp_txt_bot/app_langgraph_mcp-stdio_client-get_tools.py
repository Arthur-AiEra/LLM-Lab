...
class MCPClientManager:
    ...
    async def connect(self):
        """
        建立与 MCP Server 的连接，并使用 MultiServerMCPClient 加载工具。
        """
        # 配置服务器连接参数
        connections = {
            "txt_counter_server": {
        ...
        # 2. ✅ 使用 MultiServerMCPClient.get_tools() 获取工具
...
st.markdown("当前架构：**OpenAI SDK + LangGraph + MCP 适配器 (MultiServerMCPClient) + Streamlit**")
...
with st.sidebar:
    st.header("功能说明")
    st.info(
        "1. **MCP 适配器**：使用 `MultiServerMCPClient` 管理多个服务器并获取工具。\n2. **LangGraph**：管理复杂的 Agent 决策流。\n3. **可视化**：自动解析统计数据并绘图。")
