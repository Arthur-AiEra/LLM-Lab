...
def get_embedding(text: str, client: OpenAI) -> list:
    """使用 openai 的 text-embedding 模型为文本生成向量。"""
    try:
        # 确保文本不为空
        if not text.strip():
            return []
...
def index_and_search_documents_with_embedding():
    ...
    es_client.indices.create(
        ...
        mappings={
            "properties": {
                ...
                "content_vector": {
                    "type": "dense_vector",
                    "dims": 1536,  # 必须与 embedding 模型的维度一致
                    "index": "true",
    ...
    for filename in os.listdir(docs_folder):
        ...
        if os.path.isfile(file_path):
            try:
                ...
                if content.strip():
                    # 将长文本分块
                    text_chunks = chunk_text(content, 2000, 100)
    ...
    # b. 执行 k-NN 搜索 （k-Nearest Neighbors Search，k-近邻搜索）
    response = es_client.search(
        index=index_name,
        knn={ # 语义搜索（Semantic Search）
