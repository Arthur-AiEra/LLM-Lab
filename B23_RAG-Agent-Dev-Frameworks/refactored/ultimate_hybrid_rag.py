...
DOCS_DIR = "./docs"
DB_PATH = "ultimate_hybrid_rag/milvus_local.db"
BM25_MODEL_PATH = "ultimate_hybrid_rag/bm25_model.json"
PERSIST_DIR = "ultimate_hybrid_rag/storage"  # 👉 新增：存放文档 Hash 记录表的目录
COLLECTION_NAME = "ultimate_hybrid_knowledge_base"
...
async def main():
    ...
    print("-" * 50)
    while True:
        question = input("\n🙋 请输入你的问题 (输入 q 退出): ")  # 平安境内紧急医疗救援服务的责任免除有哪些？
        if question.lower() == 'q': break

        question = clean_text(question)
        if not question or question == "查询词": continue

        print("🤖 OpenAI LLM 正在结合 Milvus 混合检索结果进行思考...")
        try:
