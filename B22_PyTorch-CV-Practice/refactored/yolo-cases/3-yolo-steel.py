...
# model = YOLO('yolo12n.pt')  # 加载预训练模型（可选）
...
results = model.train(
    ...
    patience=50,              # 早停轮数
    save=True,               # 保存模型
    device='cpu',              # GPU设备号，如果使用CPU则设为'cpu'
    workers=8,               # 数据加载线程数
...
# 加载训练好的模型
model2 = YOLO('./runs/detect/train12/weights/best.pt')
#model2
...
if os.path.exists(test_image_dir):
    for image_name in os.listdir(test_image_dir)[:5]:  # 测试前5张图片
        ...
        # 保存预测结果
        results[0].save(os.path.join('./runs/detect/predict6', image_name))
