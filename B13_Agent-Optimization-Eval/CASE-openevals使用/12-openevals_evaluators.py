...
def evaluate_wealth_advisor():
    ...
    def predict(inputs: dict) -> dict:
        ...
        try:
            ...
            # 返回扁平化字典，确保 context 和 output 在顶层
            # 这样 openevals 的默认逻辑更容易提取到这些参数
            return {
                "output": output_text,
                "context": context,
...
- 如果没有包含任何关键词，给出 0.0 分"""
    ...
    # 2. 创建基础评估器
    base_helpfulness_evaluator = create_llm_as_judge(prompt=RAG_HELPFULNESS_PROMPT, feedback_key="helpfulness", judge=eval_llm)
    base_hallucination_evaluator = create_llm_as_judge(prompt=HALLUCINATION_PROMPT, feedback_key="hallucination", judge=eval_llm)

    # 3. 定义包装函数
    # 核心修复：手动从 run.outputs 中提取 context 并显式传递给评估器
    # 这样可以确保 openevals 内部的 get_score 能够接收到 context 参数，避免 KeyError
    def helpfulness_evaluator(run, example):
        context = run.outputs.get("context", "")
        return base_helpfulness_evaluator(
            inputs=run.inputs,
            outputs=run.outputs,
    ...
    def hallucination_evaluator(run, example):
        context = run.outputs.get("context", "")
        return base_hallucination_evaluator(
            inputs=run.inputs,
            outputs=run.outputs,
            reference_outputs=example.outputs,
            context=context
            ...
            if results_list:
                ...
                for result in results_list:
                    if "feedback_stats" in result:
                        for key, feedback in result["feedback_stats"].items():
